...
namespace FLAC {
	namespace Encoder {
        class FuzzerStream : public Stream {
            ...
            public:
                FuzzerStream(fuzzing::datasource::Datasource& ds) :
                    Stream(), ds(ds) { }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    try {
        ...
        while ( ds.Get<bool>() ) {
            {
                auto data = ds.GetVector<FLAC__int32>();
                for (size_t i = 0; i < data.size(); i++) {
                    if ( SAMPLE_VALUE_LIMIT != 0 ) {
                        if ( data[i] < -SAMPLE_VALUE_LIMIT ) {
                            data[i] = -SAMPLE_VALUE_LIMIT;
                        } else if ( data[i] > SAMPLE_VALUE_LIMIT ) {
                            data[i] = SAMPLE_VALUE_LIMIT;
                        }
                    }
                }
                const uint32_t samples = data.size() / 2;
                if ( samples > 0 ) {
